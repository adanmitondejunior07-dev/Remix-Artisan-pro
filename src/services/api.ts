  ...
  async register(data: any): Promise<{ user: User; artisan?: Artisan }> {
    try {
      ...
      const newUser: User = {
        ...
        name: data.name,
        firstName: data.firstName || (data.name.includes(' ') ? data.name.split(' ').slice(1).join(' ') : undefined),
        lastName: data.lastName || (data.name.includes(' ') ? data.name.split(' ')[0] : undefined),
        email: data.email,
        role: assignedRole,
        ...
        location_authorized: Boolean(data.location_authorized),
        showLocationPublicly: data.showLocationPublicly !== undefined ? Boolean(data.showLocationPublicly) : true,
        showEmailPublicly: data.showEmailPublicly !== undefined ? Boolean(data.showEmailPublicly) : false,
      ...
      if (data.role === 'artisan') {
        createdArtisan = await firestoreService.saveArtisan({
          ...
          city: data.city || 'Abidjan',
          country: data.country || 'Côte d’Ivoire',
          lat: data.latitude !== undefined ? Number(data.latitude) : 5.3364,
          lng: data.longitude !== undefined ? Number(data.longitude) : -4.0267,
          phone: data.phone || '+225 ',
          email: data.email,
          plan: 'Pro',
          rating: 5.0,
